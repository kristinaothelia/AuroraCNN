...
def train(model, json_file, model_name, mode, w_sampler=False, no_weights=False, ep=100, batch_size_train=8, learningRate=2e-3, stepSize=75, g=0.1):
    ...
    trainer = Trainer(model             = model,
                      ...
                      epochs            = ep,
                      model_info        = [batch_size_train, learningRate, stepSize, g, params/1e6, class_weights, model_name[-1:]],
                      save_period       = 500,
...
train(model, json_file, MN, mode='bilinear', w_sampler=False, ep=150, batch_size_train=32, learningRate=0.01, stepSize=75, g=0.1)

MN = model_name[4]
model = EfficientNet.from_name(model_name=MN, num_classes=4, in_channels=1)
train(model, json_file, MN, mode='bilinear', w_sampler=False, ep=150, batch_size_train=8, learningRate=0.01, stepSize=100, g=0.1)
train(model, json_file, MN, mode='bilinear', w_sampler=False, ep=200, batch_size_train=16, learningRate=0.01, stepSize=75, g=0.5)
